...
// we reduce the zeros so we can see numbers moving faster in testing
...
console.log("Dev/admin address", dev.publicKey.toBase58());
console.log("rewardToken address", rewardToken.publicKey.toBase58());

const connection = new Connection(clusterApiUrl("devnet"), "max");
const anchorWallet = new anchor.Wallet(dev);
...
(async () => {
  ...
  console.log("prevTotalConfigs", prevTotalConfigs);
  ...
  const ok = currentAmount === prevTotalConfigs + configs.length;
  console.log(
    ok ? "✅" : "❌",
    currentAmount,
  ...
  // Fund config reward pot so user can claim token rewards
  await allSynchronously(
    configs.map((config) => async () => {
      ...
      const tx = await provider.sendAndConfirm(mintTx);
